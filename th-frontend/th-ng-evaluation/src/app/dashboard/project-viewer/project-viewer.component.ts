...
import { Project, Path, EProgram, EvaluationConfig, EvaluationItem, Annuity } from 'th-ng-commons';
...
export class ProjectViewerComponent {
  ...
  onSubmit() {
    ...
    this.projectService.doEvaluation(requestData).subscribe(() => {
      this.toastr.success('Proyecto evaluado correctamente');
      this.getProjectInfo();
    }, error => {
      this.toastr.error('Ha ocurrido un error inesperado, por favor, inténtalo de nuevo más tarde');
    });
  }

  getRequestData(): EvaluationRequest {
    // Data extraction
    const { approved, items } = this.evaluationForm.value;
    console.log(items);
    const lUpdateEvaluation = this.getUpdateEvaluationList(items);
    const lNewEvaluation = this.getNewEvaluationList(items);
    const project = this.project;

    // EvaluationRequest instance
    return { project, approved, lNewEvaluation, lUpdateEvaluation };
  }
